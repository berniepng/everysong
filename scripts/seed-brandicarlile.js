...
// ─── Main ─────────────────────────────────────────────────────────────────────
async function seed() {
  console.log('🌱 Seeding Brandi Carlile...');
  const db = initDb();
  ...
  insertArtist.run(
    ...
    'United States',
    'Folk Rock, Americana, Alternative Country, Classic Rock',
    1981
  );

  const artist = db.prepare('SELECT id FROM artists WHERE slug = ?').get('brandicarlile');
  ...
  // Parse CSV
  const csvPath = path.join(__dirname, 'brandi_carlile_discography.csv');
  ...
  for (const { album } of albums) {
    if (!album) continue;
    const art = await fetchArt('Brandi Carlile', album);
    if (art) {
